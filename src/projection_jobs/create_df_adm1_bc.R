## RUNTIME: 6min at 40 workers and 500 GB RAM
...
# read in the cap for monthly precip deviation created in a previous script
...
# ensure target directories for ADM1 & ADM0 output files exist
dir_adm1 <- file.path(dir_output_data, "pointestimates_stagg", "adm1")
if(!dir.exists(dir_adm1)) dir.create(dir_adm1) 

dir_target_adm0 <- file.path(dir_output_data, "pointestimates_stagg", "adm0")
if(!dir.exists(dir_target_adm0)) dir.create(dir_target_adm0)  

# loop through all model runs in df_modelscen
future_pmap(.l = df_modelscen,
            .f = function(model, scenario, ensemble) {
              ...
              # save ADM1-level results out in ADM0-level chunks
              df_adm1 %>%
              ...
              # aggregate to ADM0 level using GDP weights and save out
              df_adm1 %>%
...
dir_target_adm0 %>% list.files(recursive = T, full.names = T) %>% map_dfr(read_feather) %>%
  ...
  write_feather(file.path(dir_output_data, "pointestimates_stagg", "df_global_bc.feather"))
